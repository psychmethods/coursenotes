...
ggplot(data.frame(x = c(0, 1)), aes(x)) +
  ...
  annotate("text", x = 0, y = 0.05, label = "Impossible\n(0)", hjust = 0) +
  annotate("text", x = 1, y = 0.05, label = "Certain\n(1)", hjust = 1) +
  annotate("text", x = 0.5, y = 0.05, label = "50-50 chance\n(0.5)", hjust = 0.5) +
  theme_void() +
  xlim(-0.1, 1.1)
```
...
```{r}
vague_terms <- data.frame(
  term = c("Almost Impossible", "Very Unlikely", "Unlikely", "50-50", "Likely", "Very Likely", "Almost Certain"),
  value = c(0.01, 0.1, 0.25, 0.5, 0.75, 0.9, 0.99)
)

ggplot(vague_terms, aes(x = value, y = term)) +
  geom_point(size = 3) +
  scale_x_continuous(limits = c(0, 1)) +
  labs(x = "Probability", y = "Term") +
  theme_minimal()
```
